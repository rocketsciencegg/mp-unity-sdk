...
using ErrorState = RocketScience.Bindings.BindingsRouter.Binding_ErrorState;
#endif
...
namespace RocketScience.Networking.Qos
{
    ...
    {
#if UGS_QOS_SUPPORTED
        internal BindingsRouter.Binding_NetworkAddress rawNetworkAddress;

        ushort Port => (ushort) (rawNetworkAddress.port1 | (rawNetworkAddress.port0 << 8));

        NetworkFamily Family => FromBaselibFamily((BindingsRouter.Binding_NetworkAddress_Family) rawNetworkAddress.family);
            ...
            NetworkFamily family = NetworkFamily.Ipv4)
        {
            ...
            fixed (BindingsRouter.Binding_NetworkAddress* rawAddress = &endpoint.rawNetworkAddress)
            {
                BindingsRouter.NetworkAddress_Encode(
                    rawAddress,
            ...
            return errorState.code == BindingsRouter.Binding_ErrorCode.Success && endpoint.IsValid;
        }
...
// TODO(steve): Throw an exception?
        ...
        static NetworkFamily FromBaselibFamily(BindingsRouter.Binding_NetworkAddress_Family family)
        {
            return family switch
            {
                BindingsRouter.Binding_NetworkAddress_Family.IPv4 => NetworkFamily.Ipv4,
                BindingsRouter.Binding_NetworkAddress_Family.IPv6 => NetworkFamily.Ipv6,
                _ => NetworkFamily.Invalid
            };
        }

        static BindingsRouter.Binding_NetworkAddress_Family ToBaselibFamily(NetworkFamily family)
        {
            return family switch
            {
                NetworkFamily.Ipv4 => BindingsRouter.Binding_NetworkAddress_Family.IPv4,
                NetworkFamily.Ipv6 => BindingsRouter.Binding_NetworkAddress_Family.IPv6,
                _ => BindingsRouter.Binding_NetworkAddress_Family.Invalid
            };
        }
#else

...
namespace RocketScience.Networking.Qos
{
    ...
    internal class QosResponse
    {
        ...
        {
#if UGS_QOS_SUPPORTED
            BindingsRouter.Binding_Socket_Message message = new BindingsRouter.Binding_Socket_Message();
            var buffer = new UnsafeAppendBuffer(2048, 16, Allocator.Persistent);
            var startTime = DateTime.UtcNow;
            fixed(BindingsRouter.Binding_NetworkAddress* pEndpointAddress = &endPoint.rawNetworkAddress)
            {
                ...
                var socket = new BindingsRouter.Binding_Socket_Handle { handle = socketHandle};
                ...
                {
                    errorState = new BindingsRouter.Binding_ErrorState();
                    tries++;

                    received = BindingsRouter.Socket_UDP_Recv(socket, message, 1, errorState);
                    if (received == 0)
                    {
            ...
            buffer.Dispose();
            return (Length, (int)BindingsRouter.Binding_ErrorCode.Success);
#else

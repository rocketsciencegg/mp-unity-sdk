...
using Unity.Jobs;
using static RocketScience.Bindings.BindingsRouter;

#if UGS_QOS_SUPPORTED
using System.Text;
using Unity.Baselib.LowLevel;
using RocketScience.Bindings;
using UnityEngine;
using Random = System.Random;
#endif
using RocketScience.Services.Qos.Runner;

namespace RocketScience.Networking.Qos
{
    partial struct QosJob : IQosJob
    {
        ...
        public void Execute()
        {
            ...
            Binding_ErrorCode errorCode;
            Binding_Socket_Handle socket;
            (socket, errorCode) = CreateAndBindSocket();

            if (errorCode != Binding_ErrorCode.Success)
            {
                // Can't run the job
            ...
            BindingsRouter.Socket_Close(socket);
#if UNITY_EDITOR || DEVELOPMENT_BUILD
        ...
        /// </summary>
        void ProcessServers(Binding_Socket_Handle socketHandle)
        {
        ...
        void ProcessServer(InternalQosServer server, Binding_Socket_Handle socketHandle)
        {
        ...
        /// </returns>
        BindingsRouter.Binding_ErrorCode SendQosRequests(InternalQosServer server, Binding_Socket_Handle socketHandle, ref InternalQosResult result)
        {
            ...
            do
            {
                if (QosHelper.ExpiredUtc(m_JobExpireTimeUtc))
                {
                    ...
                    return Binding_ErrorCode.Timeout;
                }
                ...
                if ((Binding_ErrorCode)errorCode != Binding_ErrorCode.Success)
                {
                    Debug.LogError($"QosJob: send returned error code {(Binding_ErrorCode)errorCode}, can't continue");
                    return (Binding_ErrorCode)errorCode;
                }
        ...
        void RecvQosResponsesTimed(NetworkEndPoint addr, DateTime deadline, Binding_Socket_Handle socketHandle, bool wait)
        {
#if UNITY_EDITOR || DEVELOPMENT_BUILD
        ...
        void RecvQosResponses(NetworkEndPoint addr, DateTime deadline, Binding_Socket_Handle socketHandle, bool wait)
        {
        ...
        /// </returns>
        static unsafe (Binding_Socket_Handle, Binding_ErrorCode) CreateAndBindSocket()
        {
            var errorState = new BindingsRouter.Binding_ErrorState();

            // Create the socket handle.
            var socketHandle = BindingsRouter.Socket_Create(BindingsRouter.Binding_NetworkAddress_Family.IPv4,
                BindingsRouter.Binding_Socket_Protocol.UDP, errorState);

            if (errorState.code != BindingsRouter.Binding_ErrorCode.Success)
            {
